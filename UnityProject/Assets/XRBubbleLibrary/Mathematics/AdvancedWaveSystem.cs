...
namespace XRBubbleLibrary.Mathematics
{
    ...
    public class AdvancedWaveSystem : MonoBehaviour
    {
        ...
        // AI integration (using dependency inversion pattern)
        private IBiasField currentBiasField;
        private IWaveOptimizer waveOptimizer;
        private IBiasFieldProvider biasFieldProvider;
        ...
        void InitializeWaveSystem()
        {
            ...
            // Initialize AI optimization using dependency inversion pattern
            if (enableAIOptimization)
            {
                waveOptimizer = WaveOptimizationRegistry.GetOptimizer();
                biasFieldProvider = BiasFieldRegistry.GetProvider();
                
                if (waveOptimizer == null || biasFieldProvider == null)
                {
                    Debug.LogWarning("AI optimization services not available. Using mathematical-only positioning.");
                    enableAIOptimization = false;
                }
                else
                {
                    Debug.Log("AI optimization services initialized successfully.");
                }
            }
        ...
        /// Uses dependency inversion pattern to avoid direct AI assembly dependencies
        /// </summary>
        public async Task<float3[]> GenerateOptimalBubblePositions(int bubbleCount, float3 userPosition)
        {
            ...
            // 2. Apply AI optimization if available through interface
            if (enableAIOptimization && WaveOptimizationRegistry.IsOptimizationAvailable)
            {
                try
                {
                    var optimizer = WaveOptimizationRegistry.GetOptimizer();
                    if (optimizer != null)
                    {
                        return await optimizer.OptimizeWavePositions(mathematicalPositions, userPosition);
                    }
                }
                catch (System.Exception ex)
                {
                    Debug.LogWarning($"AI optimization failed, falling back to mathematical positioning: {ex.Message}");
                }
            }
        ...
        /// Uses interface pattern for loose coupling with AI assembly
        /// </summary>
        float3[] ApplyAIBiasToPositions(float3[] mathematicalPositions, IBiasField biasField)
        {
            if (biasField == null || !biasField.IsValid)
            {
                return mathematicalPositions;
            }
            ...
            for (int i = 0; i < mathematicalPositions.Length; i++)
            {
                ...
                float3 bias = biasField.GetBias(i) * aiBiasStrength * biasField.Strength;
        ...
        /// Uses dependency inversion pattern to access AI services
        /// </summary>
        async void UpdateAIBiasField()
        {
            if (!BiasFieldRegistry.IsAvailable) return;
            
            try
            {
                var provider = BiasFieldRegistry.GetProvider();
                if (provider != null)
                {
                    var userPosition = Camera.main?.transform.position ?? float3.zero;
                    currentBiasField = await provider.GenerateBiasField(userPosition, activeBubbleCount);
                }
            }
            catch (System.Exception ex)
            {
        ...
        public WaveSystemMetrics GetPerformanceMetrics()
        {
            return new WaveSystemMetrics
            {
                ...
                aiOptimizationEnabled = enableAIOptimization && WaveOptimizationRegistry.IsOptimizationAvailable,
